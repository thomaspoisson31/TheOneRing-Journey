...
App.features.seasons = (function() {
    ...
    function updateCalendarUI() {
        const status = DOM.get('calendar-status-text');
        const selector = DOM.get('calendar-date-selector');
        const monthSelect = DOM.get('calendar-month-select');
        const manualSeasons = DOM.get('manual-seasons-section');
        const info = DOM.get('season-mode-info');
        ...
        if (AppState.calendarData && AppState.calendarData.length > 0) {
            ...
            if (AppState.currentCalendarDate) {
                ...
                if (monthIndex >= 0) {
                    ...
                    updateDaySelector();
                    DOM.get('calendar-day-select').value = AppState.currentCalendarDate.day;
                }
            }
