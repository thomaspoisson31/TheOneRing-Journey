...
App.features.journey = (function() {
    ...
    function updateDiscoveriesChronologically() {
        ...
        for (let i = 0; i < AppState.journeyPath.length; i++) {
            ...
            AppState.regionsData.regions.forEach(region => {
                if (region.points && App.utils.helpers.isPointInPolygon(currentPoint, region.points)) {
    ...
    function displayJourneyInfo() {
        ...
        if (regions.length > 0) {
            ...
            traversedRegionsList.innerHTML = regions.map(r => `<div>${escapeHtml(r.name)}</div>`).join('');
        } else {
        ...
        if (locations.length > 0) {
            ...
            nearbyLocationsList.innerHTML = locations.map(l => `<div>${escapeHtml(l.name)} <span class="text-gray-400 text-xs">(${l.proximityType})</span></div>`).join('');
        } else {
    ...
    function start(event) {
        ...
        AppState.startPoint = App.utils.helpers.getCanvasCoordinates(event);
    ...
    function draw(event) {
        ...
        const currentPoint = App.utils.helpers.getCanvasCoordinates(event);
