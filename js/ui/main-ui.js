App.ui.main = (function() {
    ...
    function displaySavedContexts(contexts) {
        ...
        contexts.forEach(context => {
            ...
            contextEl.innerHTML = `
                <span class="text-sm">${escapeHtml(context.name)}</span>
    ...
    function setupEventListeners() {
        ...
        DOM.getElementById('draw-mode').addEventListener('click', (e) => toggleDrawingMode(e.currentTarget));
        DOM.getElementById('add-location-mode').addEventListener('click', (e) => toggleAddLocationMode(e.currentTarget));
        DOM.getElementById('add-region-mode').addEventListener('click', (e) => toggleAddRegionMode(e.currentTarget));

        DOM.getElementById('erase').addEventListener('click', App.features.journey.erase);
        DOM.getElementById('export-locations').addEventListener('click', App.api.dataStorage.exportUnifiedData);
        DOM.getElementById('import-locations').addEventListener('click', () => DOM.getElementById('import-file-input').click());
        DOM.getElementById('import-file-input').addEventListener('change', App.api.dataStorage.importUnifiedData);
        ...
        DOM.viewport.addEventListener('mousedown', (e) => { if (AppState.isDrawingMode) App.features.journey.start(e); });
        DOM.viewport.addEventListener('mousemove', (e) => { if (AppState.isDrawingMode) App.features.journey.draw(e); });
        DOM.viewport.addEventListener('mouseup', (e) => { if (AppState.isDrawingMode) App.features.journey.stop(e); });
        DOM.viewport.addEventListener('mouseleave', (e) => { if (AppState.isDrawingMode) App.features.journey.stop(e); });
