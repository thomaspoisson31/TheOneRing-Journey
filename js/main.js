// --- Data ---
        ...
        function showLocationContent(location) {
            ...
            // L'onglet "Tables aléatoires" (images) a été supprimé - plus de contenu à gérer ici
        ...
        function updateDiscoveriesChronologically() {
            ...
            for (let i = 0; i < journeyPath.length; i++) {
                ...
                // Check for regions we're entering
                pointRegions.forEach(regionName => {
                    if (!currentRegions.has(regionName)) {
                        // Entering a new region
                        if (!traversedRegions.has(regionName)) {
                            traversedRegions.add(regionName);
                            // Add to chronological discoveries if not already present
                            if (!journeyDiscoveries.some(d => d.name === regionName && d.type === 'region')) {
                        ...
                        regionSegments.set(regionName, {entryIndex: i, exitIndex: i});
                    }
                });
...
LL'onglet "Tables aléatoires" (images) a été supprimé - plus de contenu à gérer ici
</changes>
